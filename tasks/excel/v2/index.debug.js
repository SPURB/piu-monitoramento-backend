...
/*
  ...
  criará arquivos de imagem com screenshots das urls em output/v2/checagem
*/
async function checkLinks (files) {
	const browser = await puppeteer.launch({ headless: false })
  const page = await browser.newPage()

  const outputPath = file => `${path.resolve(__dirname, '../../../')}/output/v2/checagem/${file}`
...
const files = read('input/piu-monitoramento_v2.xlsx', 'arquivos_tramitacao', 'sheet_to_json')
